...
`quoter-api-backend` service over HTTP; this app holds no server-side keys. The
...
Run `quoter-api-backend` alongside it on port 3001 (its default), which matches
...
copies in `quoter-api-backend` for its verification script; this repo is canonical.
