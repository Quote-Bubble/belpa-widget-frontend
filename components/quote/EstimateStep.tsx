...
export function EstimateStep({
  ...
  return (
    ...
    // (!pb-6).
    //
    // The old comment here claimed the content was "kept lean enough to fit as
    // a single page, no overflow, no scrollbar". That held for the collapsed
    // estimate and stopped holding the moment the breakdown opens — line items,
    // total, and a paragraph of rate sources run well past the panel on a tall
    // roof. It was `justify-center`, which centres the overflow, so the heading
    // and address were cut off the TOP with no way to scroll back to them.
    //
    // Now the shell scrolls. Centring moves to `m-auto` on the wrapper below
    // rather than justify-center, because auto margins resolve to zero once
    // free space is negative — whereas justify-center keeps centring and puts
    // the start of the content permanently out of reach of the scrollbar.
    <StepShell className={variant === "card" ? "!pb-6 overflow-y-auto" : ""}>
      <div className="m-auto w-full">
        ...
        <div className="shrink-0 text-center">
          ...
          >
            ...
            <InfoTip>
              Your roofer will call to confirm the final price after a free
              survey , this range is indicative, not a contract price.
            </InfoTip>
          </h1>
          <p
